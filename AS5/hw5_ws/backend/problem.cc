...
void Problem::MakeHessian() {
    ...
    for (auto &edge: edges_) {
        ...
        assert(jacobians.size() == verticies.size()); //如果表达式为假,终止程序
...
void Problem::TestMarginalize() {
    ...
    double delta1 = 0.1 * 0.1;   //方差!要把delta2给边缘化
    ...
    // |Arr  Arm  |
    // |Amr  Amm  |  此处是把H_prior(Amm)放在了右下角, 计算边际概率的公式不变, 只是对应的矩阵块位置改变了
