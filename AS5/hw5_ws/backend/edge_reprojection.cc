...
void EdgeReprojection::ComputeResidual() {
    ...
    Qd Qi(param_i[6], param_i[3], param_i[4], param_i[5]); //表示旋转的四元数
    Vec3 Pi = param_i.head<3>();  //表示位置xyz

    VecX param_j = verticies_[2]->Parameters();
    Qd Qj(param_j[6], param_j[3], param_j[4], param_j[5]);
    Vec3 Pj = param_j.head<3>();

    Vec3 pts_camera_i = pts_i_ / inv_dep_i;   //ow特征点在cami归一化平面内的坐标
    Vec3 pts_imu_i = qic * pts_camera_i + tic;   //ow特征点i从cam归一化平面转到imu系的坐标
    Vec3 pts_w = Qi * pts_imu_i + Pi;   //特征点从imu系转到世界坐标系下的坐标
    Vec3 pts_imu_j = Qj.inverse() * (pts_w - Pj);  //从世界坐标系转到对应camj的imu系
    Vec3 pts_camera_j = qic.inverse() * (pts_imu_j - tic);  //从imu到相机j
...
//    reduce = information_ * reduce;
    ...
    ///------------- check jacobians -----------------
    ///思路: 因为不确定雅克比是否正确,所以用数值进行验算
    ///在某一维变量上增加一个很小的量,如10e-6 看残差变量, 求出一个数值的雅科比 并与解析求出来的进行比较
//    {
